...
1. `data/core/sensory_objects.jsonl`에서 sensory object를 로드합니다.
...
- candidate sensory object 중 소규모 curated batch를 main ontology에 승격했습니다.
- 승격 object별 phrase cue와 negative cue를 추가했습니다.
- v1.5 ontology expansion regression case를 추가했습니다.
- promoted/deferred candidate review status를 갱신했습니다.
- holdout과 parser scoring logic은 변경하지 않았습니다.

자세한 내용은 [Curated Ontology Expansion Batch 1](docs/curated_ontology_expansion_v1_5.md)를 참고하세요.

## v1.5.1 — Ontology Taxonomy Cleanup & Data Governance

ontology 구조와 data 관리 체계를 정리한 governance 버전입니다. sensory object, parser scoring, holdout set은 변경하지 않았습니다.

**object_role 분류 체계 도입:**

전체 58개 sensory object에 `object_role`을 부여했습니다. 9개 역할로 분류합니다.

| object_role | 설명 |
| --- | --- |
| `material_anchor` | 직물, 가죽, 물리적 소재 |
| `organic_anchor` | 식물, 목재, 생물 기원 소재 |
| `mineral_anchor` | 돌, 크리스탈, 금속, 무기물 |
| `atmosphere_anchor` | 장면, 환경, 시간대, 날씨 |
| `texture_finish_anchor` | 촉감 또는 잔류 질감 중심 감각 |
| `light_density_anchor` | 광도, 무게감, 밀도 중심 감각 |
| `rendering_anchor` | 시각적 렌더링·선명도 중심 감각 |
| `taste_feel_anchor` | 식품, 음료 맛/질감 중심 감각 |

**주요 변경사항:**

- v1.5에서 승격된 3개 object의 family 수정: Creamy / Tea / Gourmand → `Food_Texture`
- phrase cue 미보유 6개 object에 cue 추가 (58/58 전체 커버리지 달성)
- `data/` 폴더를 목적별 5개 하위 폴더로 재편성
- `src/sensory_atlas/paths.py` 신규 추가 — 모든 data 경로를 단일 레지스트리로 관리
- `data/vocabulary/axis_mapping.json` 신규 추가 — 확장 axis 17개와 parser canonical axis 10개를 연결하는 mapping layer
- `data/data_manifest.json` 신규 추가 — data 파일 전체 목록과 목적 기술
- `docs/ontology_taxonomy_cleanup_v1_5_1.md` 신규 추가 — 모호한 object role 4개에 대한 결정 근거와 향후 검토 조건 기술
- `docs/sensory_object_criteria.md` 신규 추가 — sensory object 선정 기준 및 object_role 정의 문서화
- 온톨로지 구조 검증 테스트 22개 추가
...
│   │   └── domain_mapping.json
│   ├── workflow/                          ← candidate review 결과물
│   │   ├── sensory_object_candidates.jsonl
│   │   ├── candidate_review_status.jsonl
│   │   └── curated_candidate_shortlist_v1_5.jsonl
│   ├── regression/                        ← 실패 케이스·expansion 추적
│   │   ├── dev_failure_cases.jsonl
│   │   └── ontology_expansion_v1_5_cases.jsonl
│   └── data_manifest.json                 ← 전체 data 파일 목적·schema 기술
├── docs/
│   ├── ontology_taxonomy_cleanup_v1_5_1.md
│   ├── sensory_object_criteria.md
│   ├── data_inventory.md
│   ├── architecture.md
...
├── src/
│   └── sensory_atlas/
│       ├── paths.py                       ← 중앙 path 레지스트리 (v1.5.1 신규)
│       ├── parser.py
│       ├── matcher.py
│       ├── schema.py
│       ├── loaders.py
...
- `parser.py`의 하드코딩 경로를 `paths.py`로 마이그레이션하여 `data/` root compat copy 제거
...
- [Ontology Taxonomy Cleanup v1.5.1](docs/ontology_taxonomy_cleanup_v1_5_1.md)
- [Sensory Object Criteria](docs/sensory_object_criteria.md)
